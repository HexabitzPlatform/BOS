...
static portBASE_TYPE prvRunTimeStatsCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE pingCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE bootloaderUpdateCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
#ifndef __N
static portBASE_TYPE exploreCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
#endif
static portBASE_TYPE resetCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE nameCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE groupCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE statusCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE infoCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE scastCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE addbuttonCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE removebuttonCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE setCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE getCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE defaultCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE timeCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE dateCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE setBaudrateCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE uuidCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE idcodeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE flashsizeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE snipCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString);
static portBASE_TYPE actSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE pauseSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE delSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE bridgeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
static portBASE_TYPE unbridgeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString);
...
static portBASE_TYPE prvRunTimeStatsCommand(int8_t *pcWriteBuffer,
	...
	vTaskGetRunTimeStats(
			((char*) pcWriteBuffer) + strlen((char*) pcStatsTableHeader));
...
static portBASE_TYPE bootloaderUpdateCommand(int8_t *pcWriteBuffer,
	...
	static const int8_t *pcMessage =
			(int8_t*) "Update firmware for module %d\n\r";
	...
	static portBASE_TYPE xParameterStringLength1, xParameterStringLength2,
		...
		*((unsigned long*) 0x20007FF0) = 0xDEADBEEF;
		...
		if (!strncmp((const char*) pcParameterString1, "via",
			...
			pcParameterString2 = (int8_t*) FreeRTOS_CLIGetParameter(
					pcCommandString, 2, &xParameterStringLength2);
			pcParameterString3 = (int8_t*) FreeRTOS_CLIGetParameter(
					pcCommandString, 3, &xParameterStringLength3);
...
static portBASE_TYPE nameCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcMessageKey =
			(int8_t*) "%s is a reserved BOS keyword! Please use a different alias\n\r";
	static const int8_t *pcMessageAlias =
			(int8_t*) "%s is already used! Please use a different alias\n\r";
	static const int8_t *pcMessageCmd =
			(int8_t*) "%s is an existing CLI command! Please use a different alias\n\r";
...
static portBASE_TYPE groupCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcMessageWrongModule =
			(int8_t*) "%s is a wrong module ID or alias\n\r";
	static const int8_t *pcMessageOKnew =
			(int8_t*) "] added to new group %s\n\r";
	static const int8_t *pcMessageOKexist =
			(int8_t*) "] added to existing group %s\n\r";
	static const int8_t *pcMessageKey =
			(int8_t*) "%s is a reserved BOS keyword! Please use a different alias\n\r";
	static const int8_t *pcMessageAlias =
			(int8_t*) "%s is already used! Please use a different alias\n\r";
	static const int8_t *pcMessageCmd =
			(int8_t*) "%s is an existing CLI command! Please use a different alias\n\r";
	static const int8_t *pcMessageNoModules =
			(int8_t*) "Please enter some module IDs to add to %s\n\r";
...
static portBASE_TYPE infoCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	if (result == BOS_ERR_NoResponse) {
		sprintf((char*) pcWriteBuffer,
				"Could not read ports direction for some modules! Please try again\n\r");
		writePxMutex(PcPort, (char*) pcWriteBuffer,
				strlen((char*) pcWriteBuffer), cmd50ms, HAL_MAX_DELAY);
	}
	sprintf((char*) pcWriteBuffer, " ");
...
static portBASE_TYPE scastCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcMessage =
			(int8_t*) "Activating a %s single-cast DMA stream from P%d in module %s to P%d in module %s. The stream will deactivate after %d bytes or %d ms\n\r";
...
static portBASE_TYPE addbuttonCommand(int8_t *pcWriteBuffer,
	...
	static const int8_t *pcMessage =
			(int8_t*) "A new %s button named B%d was defined at port P%d\n\r";
...
static portBASE_TYPE removebuttonCommand(int8_t *pcWriteBuffer,
	...
	static const int8_t *pcMessage =
			(int8_t*) "Button B%d was removed from port P%d\n\r";
...
static portBASE_TYPE setCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcMessageWrongValue = (int8_t*) "Wrong value!\n\r";
	static const int8_t *pcMessageCLI1 =
			(int8_t*) "\nYou must restart to enable the new baudrate.\n\r";
	static const int8_t *pcMessageCLI2 =
			(int8_t*) "This affects all ports. If you change this value from default, \
	...
	if (!strncmp((const char*) pcParameterString1, "bos.", 4)) {
		...
		if (!strncmp((const char*) pcParameterString1 + 4, "response",
			...
			if (temp16 >= 1 && temp16 <= USHRT_MAX) {
				...
				"singleclicktime", xParameterStringLength1 - 4)) {
			temp16 = atoi((const char*) pcParameterString2);
			if (temp16 >= 1 && temp16 <= USHRT_MAX) {
				...
				"mininterclicktime", xParameterStringLength1 - 4)) {
			temp16 = atoi((const char*) pcParameterString2);
			if (temp16 >= 1 && temp16 <= UCHAR_MAX) {
				...
				"maxinterclicktime", xParameterStringLength1 - 4)) {
			temp16 = atoi((const char*) pcParameterString2);
			if (temp16 >= 1 && temp16 <= UCHAR_MAX) {
		...
		if (result == BOS_OK) {
			if (temp81 > 23 || temp82 > 59 || temp83 > 59)
				...
				result = BOS_CalendarConfig(BOS.date.month, BOS.date.day,
						BOS.date.year, BOS.date.weekday, temp83, temp82, temp81,
						temp84, BOS.daylightsaving);
			}
		}
		...
		if (result == BOS_OK) {
			if (temp83 < 1 || temp83 > 31 || temp16 < 2000 || temp16 > 2100)
				...
				result = BOS_CalendarConfig(temp82, temp83, temp16, temp81,
						BOS.time.seconds, BOS.time.minutes, BOS.time.hours,
...
static portBASE_TYPE getCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
	...
	static const int8_t *pcMessageWrongParam = (int8_t*) "Wrong parameter!\n\r";
	static const int8_t *pcMessageWrongValue =
			(int8_t*) "%s is set to a wrong value!\n\r";
	static const int8_t *pcMessageGroupDoesNotExist =
			(int8_t*) "%s group does not exist!\n\r";
	static const int8_t *pcMessageGroupExists =
			(int8_t*) "Group %s members:\n\r";
...
static portBASE_TYPE defaultCommand(int8_t *pcWriteBuffer,
	...
	static const int8_t *pcMessageOKParams =
			(int8_t*) "All parameters set to default values\n\r";
	static const int8_t *pcMessageOKArray =
			(int8_t*) "Current array topology was removed. Please reboot all modules\n\r";
	static const int8_t *pcMessageWrongValue = (int8_t*) "Wrong value!\n\r";
...
static portBASE_TYPE timeCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString) {
	static const int8_t *pcMessage24 =
			(int8_t*) "Current time is %02d:%02d:%02d-%03d\n\r";
	static const int8_t *pcMessage12 =
			(int8_t*) "Current time is %02d:%02d:%02d-%03d %s\n\r";
...
static portBASE_TYPE setBaudrateCommand(int8_t *pcWriteBuffer,
	...
	static const int8_t *pcMessageOK =
			(int8_t*) "Baudrate for port P%d was set to %d\r\n";
	static const int8_t *pcMessageWrongParam = (int8_t*) "Wrong parameter!\r\n";
...
static portBASE_TYPE flashsizeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageFLASH =
			(int8_t*) "MCU Flash size is %d Kbytes\n\r";
...
static portBASE_TYPE snipCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen,
		const int8_t *pcCommandString) {
	static const int8_t *pcMessageSnipWelcome =
			(int8_t*) "The following Command Snippets are stored in memory:\n\n\r";
	static const int8_t *pcMessageSnipAction =
			(int8_t*) "To delete a Snippet, type: del-snip x\n\rTo activate a Snippet, type: \
act-snip x\n\rTo pause a Snippet, type: pause-snip x\n\n\rwhere x is the Snippet number from the list\n\r";
	...
	static const int8_t *pcMessageSnipButtonEventClicked =
			(int8_t*) "%sif b%d.clicked";
	static const int8_t *pcMessageSnipButtonEventDblClicked =
			(int8_t*) "%sif b%d.double clicked";
	static const int8_t *pcMessageSnipButtonEventPressed =
			(int8_t*) "%sif b%d.pressed for %d";
	static const int8_t *pcMessageSnipButtonEventReleased =
			(int8_t*) "%sif b%d.released for %d";
	static const int8_t *pcMessageSnipModuleParamConst =
			(int8_t*) "%sif %s %s %.1f";
	...
	writePxMutex(PcPort, (char*) pcMessageSnipWelcome,
			strlen((char*) pcMessageSnipWelcome), cmd50ms, HAL_MAX_DELAY);
	...
	for (uint8_t s = 0; s < numOfRecordedSnippets; s++) {
		...
		case SNIP_COND_BUTTON_EVENT:
			...
			case CLICKED:
				sprintf((char*) pcWriteBuffer,
						(char*) pcMessageSnipButtonEventClicked,
						(char*) pcWriteBuffer, snippets[s].cond.buffer1[0],
						snippets[s].cmd);
				break;
			case DBL_CLICKED:
				sprintf((char*) pcWriteBuffer,
						(char*) pcMessageSnipButtonEventDblClicked,
						(char*) pcWriteBuffer, snippets[s].cond.buffer1[0],
						snippets[s].cmd);
				break;
			...
			case PRESSED_FOR_X3_SEC:
				sprintf((char*) pcWriteBuffer,
						(char*) pcMessageSnipButtonEventPressed,
						(char*) pcWriteBuffer, snippets[s].cond.buffer1[0],
						snippets[s].cond.buffer1[2], snippets[s].cmd);
				break;
			case RELEASED_FOR_Y1_SEC:
			case RELEASED_FOR_Y2_SEC:
			case RELEASED_FOR_Y3_SEC:
				sprintf((char*) pcWriteBuffer,
						(char*) pcMessageSnipButtonEventReleased,
						(char*) pcWriteBuffer, snippets[s].cond.buffer1[0],
						snippets[s].cond.buffer1[2], snippets[s].cmd);
				break;
			default:
		...
		case SNIP_COND_MODULE_PARAM_CONST:
			...
			sprintf((char*) pcWriteBuffer,
					(char*) pcMessageSnipModuleParamConst,
					(char*) pcWriteBuffer,
...
static portBASE_TYPE actSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageOK =
			(int8_t*) "Snippet was activated. Type snip to view updated list\n\r";
	static const int8_t *pcMessageWrong =
			(int8_t*) "The Snippet number was not found\n\r";
	int8_t *pcParameterString1;
	portBASE_TYPE xParameterStringLength1 = 0;
	BOS_Status result = BOS_OK;
...
static portBASE_TYPE pauseSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageOK =
			(int8_t*) "Snippet was paused. Type snip to view updated list\n\r";
	static const int8_t *pcMessageWrong =
			(int8_t*) "The Snippet number was not found\n\r";
	int8_t *pcParameterString1;
	portBASE_TYPE xParameterStringLength1 = 0;
...
static portBASE_TYPE delSnipCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageOK =
			(int8_t*) "Snippet was deleted. Type snip to view updated list\n\r";
	static const int8_t *pcMessageWrong =
			(int8_t*) "The Snippet number was not found\n\r";
	int8_t *pcParameterString1;
	portBASE_TYPE xParameterStringLength1 = 0;
...
static portBASE_TYPE bridgeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageOK =
			(int8_t*) "P%d and P%d are bridged together\n\r";
...
static portBASE_TYPE unbridgeCommand(int8_t *pcWriteBuffer,
		size_t xWriteBufferLen, const int8_t *pcCommandString) {
	static const int8_t *pcMessageOK =
			(int8_t*) "P%d and P%d are un-bridged\n\r";
	static const int8_t *pcMessageWrong = (int8_t*) "Wrong syntax\n\r";
...
static portBASE_TYPE ADCReadCommand(int8_t *pcWriteBuffer,
	...
	if (strcmp((char*) pcParameterString1, "2") == 0
			|| strcmp((char*) pcParameterString1, "3") == 0) {

		ADCports = (uint8_t) atol((char*) pcParameterString1 + 1);

		if (strcmp((char*) pcParameterString2, "top") == 0
				|| strcmp((char*) pcParameterString2, "bottom") == 0) {

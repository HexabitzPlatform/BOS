...
//	uint8_t perviousCharacter =0;
//	uint8_t numCommandParameters =0;
//	uint8_t digitTheCommand =0;
//	uint8_t finalMatrixIndex =0;
//	uint8_t twoDMatrixIndex =0;
//	uint8_t counter =0;
//	uint8_t desiredArray =0;
//	uint8_t nonProcessingParameterIndex =0;
//	uint8_t processingParameterIndex =0;
...
uint32_t ulMainGetRunTimeCounterValue(void){
	uint32_t ulSysTickCounts =0, ulTickCount =0, ulReturn =0;
	...
	/* NOTE: There are potentially race conditions here.
	 * However, it is used anyway to keep the examples simple,
	 * and to avoid reliance on a separate timer peripheral. */

	/* Read the current SysTick count (time elapsed since last tick) */
	ulSysTickCounts =ulSysTickReloadValue - *pulCurrentSysTickCount;
	
	/* Get the number of FreeRTOS OS ticks */
	ulTickCount =xTaskGetTickCountFromISR();
	
	/* Check if a SysTick interrupt is pending (indicating an overflow) */
	if((*pulInterruptCTRLState & ulSysTickPendingBit) != 0UL){
		/* Increment tick count manually as it hasn't been updated yet */
		ulTickCount++;
		
		/* Read SysTick again to prevent race condition */
		ulSysTickCounts =ulSysTickReloadValue - *pulCurrentSysTickCount;
	}
	
	/* Convert tick count to tenths of a millisecond */
	ulReturn =(ulTickCount * 10UL);
	
	 /* Add fractional SysTick time converted to tenths of a millisecond */

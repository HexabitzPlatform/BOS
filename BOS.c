...
DEF_BUTTON_DEBOUNCE , .buttons.singleClickTime = DEF_BUTTON_CLICK ,
    .buttons.minInterClickTime = DEF_BUTTON_MIN_INTER_CLICK ,
    .buttons.maxInterClickTime = DEF_BUTTON_MAX_INTER_CLICK ,
    .daylightsaving =DAYLIGHT_NONE , .hourformat =24 , .disableCLI = false};
uint16_t myPN = modulePN;
uint8_t indMode =IND_OFF;

/* Define module PN strings [available PNs+1][5 chars] */
const char modulePNstring[NUM_OF_MODULE_PN][6] ={"" , "H01R0" , "P01R0" ,
    "H23R0" , "H23R1" , "H07R3" , "H08R6" , "P08R6" , "H09R0" , "H1BR6" , "H12R0" ,
    "H13R7" , "H0FR1" , "H0FR6" , "H0FR7" , "H1AR2" , "H0AR9" , "H1DR1" , "H1DR5" ,
    "H0BR4" , "H18R0" , "H26R0" , "H15R0" , "H10R4"};

/* Define BOS keywords */
static const char BOSkeywords[NumOfKeywords][4] ={"me" , "all" , "if" , "for"};

const char *monthStringAbreviated[] ={"Jan" , "Feb" , "Mar" , "Apr" , "May" ,
    "Jun" , "Jul" , "Aug" , "Sep" , "Oct" , "Nov" , "Dec"};
static const char *weekdayString[] ={"Monday" , "Tuesday" , "Wednesday" ,
...
BOS_Status SetupDMAStreams(uint8_t direction, uint32_t count, uint32_t timeout,
...
BOS_Status WriteToRemote(uint8_t module, uint32_t localAddress,
...
BOS_Status User_MessagingParser(uint16_t code, uint8_t port, uint8_t src,
...
BOS_Status SaveEEgroup(void){
	...
	for(i =0; i < MaxNumOfGroups; i++)// MaxNumOfGroups group aliases
		...
		if(groupAlias[i][0]) {
			for(uint8_t j =1; j <= MaxLengthOfAlias; j +=2) {
				temp =(uint16_t) (groupAlias[i][j - 1] << 8)
				    + groupAlias[i][j];
		...
		if(!status1)// This variable exists
		{
			...
			if( ( (temp8 >> 4) == i + 1) && ( (temp8 & 0x0F) != NONE))// This is same port and button type is not none
			{
				...
				SetButtonEvents(i + 1,
				    ...
				    button[i + 1].pressedX1Sec,button[i + 1].pressedX2Sec,
				    button[i + 1].pressedX3Sec,button[i + 1].releasedY1Sec,
				    button[i + 1].releasedY2Sec,
				    button[i + 1].releasedY3Sec,BUTTON_EVENT_MODE_CLEAR);
			}
		}
	}
...
BOS_Status WriteToRemote(uint8_t module, uint32_t localAddress,
	...
	if(remoteAddress < FLASH_BASE) {
		...
		switch(format){
			...
			case FMT_UINT16:
			messageParams[2] =
			    (uint8_t) ( (*(__IO uint16_t*) localAddress) >> 0);
			messageParams[3] =
			    (uint8_t) ( (*(__IO uint16_t*) localAddress) >> 8);
			...
			case FMT_UINT32:
			messageParams[2] =
			    (uint8_t) ( (*(__IO uint32_t*) localAddress) >> 0);
			messageParams[3] =
			    (uint8_t) ( (*(__IO uint32_t*) localAddress) >> 8);
			messageParams[4] =
			    (uint8_t) ( (*(__IO uint32_t*) localAddress) >> 16);
			messageParams[5] =
			    (uint8_t) ( (*(__IO uint32_t*) localAddress) >> 24);
			...
			messageParams[4] =
			    (uint8_t) ( (*(__IO int32_t*) localAddress) >> 16);
			messageParams[5] =
			    (uint8_t) ( (*(__IO int32_t*) localAddress) >> 24);
			SendMessageToModule(module,CODE_WRITE_REMOTE,6);
				break;
			case FMT_FLOAT:
 ...
 */
uint32_t* ReadRemoteVar(uint8_t module, uint32_t remoteAddress,
...
/* --- Read a memory address from a remote module. 
 ...
 timeout: Read timeout in msec.
 */
uint32_t* ReadRemoteMemory(uint8_t module, uint32_t remoteAddress,
...
/* --- Read a parameter from a remote module. 
 ...
 timeout: Read timeout in msec.
 */
uint32_t* ReadRemoteParam(uint8_t module, char *paramString,
    varFormat_t *remoteFormat, uint32_t timeout){
	/* Reset local buffer */
...
/* --- Write a value to a remote module. 
 ...
 timeout: Write confirmation timeout in msec. Use 0 to disable confirmation.
 */
BOS_Status WriteRemote(uint8_t module, uint32_t localAddress,
    uint32_t remoteAddress, varFormat_t format, uint32_t timeout){
	return WriteToRemote(module,localAddress,remoteAddress,format,timeout,
	    0);
}
...
/* --- Write a value to a remote module and force full-page erase when writing to Flash. 
 ...
 timeout: Write confirmation timeout in msec. Use 0 to disable confirmation.
 */
BOS_Status WriteRemoteForce(uint8_t module, uint32_t localAddress,
    uint32_t remoteAddress, varFormat_t format, uint32_t timeout){
	return WriteToRemote(module,localAddress,remoteAddress,format,timeout,
	    1);
}
...
/*  Write three variables to ModBus module:
 ...
 BOS module ID is 2.
 */
BOS_Status WriteToMBModule(uint8_t dst, uint8_t rank, float var1, float var2,
    float var3){
	BOS_Status result =BOS_OK;

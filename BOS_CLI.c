...
void CLI_CommandParser(uint8_t port, bool enableOutput, int8_t *cInputString,
...
void prvCLITask(void *pvParameters){
	...
	static int8_t cInputString[cmdMAX_INPUT_SIZE],
	    cLastInputString[cmdMAX_INPUT_SIZE];
	uint16_t chr =0;
...
//configASSERT( xReturned );
	...
	sprintf(pcWelcomePortMessage,
	    "Connected to module %d (%s), port P%d.\n\n\r>",myID,
		...
		/* Parse a normal Command */
			...
			if(loc != NULL
				...
				if(id == myID) {
					...
					sprintf((char*) pcOutputString,
					    "Wrong module name! Please try again.\n\r");
					xReturned = pdFALSE;
				}
				else if(id == BOS_ERR_WrongID) {
					sprintf((char*) pcOutputString,
					    "Wrong module ID! Please try again.\n\r");
					xReturned = pdFALSE;
				}
...
//sprintf( ( char * ) pcOutputString, "Module %d is not reachable.\n\r", m);	
					...
					xReturned =FreeRTOS_CLIProcessCommand(
					    ...
					    configCOMMAND_INT_MAX_OUTPUT_SIZE);
					sprintf((char*) pcOutputString,
					    "%sMulticast Command forwarded to group %s\n\r",
					    pcOutputString,idString);
				}
				else {
					...
					if(BOS.response == BOS_RESPONSE_ALL) {
						...
						if(responseStatus != BOS_OK) {
							++timedoutMsg;
							sprintf((char*) pcOutputString,
							    "%sModule %d is not reachable.\n\r",
							    (char*) pcOutputString,id);
						}
					}
					xReturned = pdFALSE;
				}
	...
	writePxMutex(port,pcEndOfCommandOutputString,
	    strlen(pcEndOfCommandOutputString),cmd50ms,HAL_MAX_DELAY);
...
// Add two more bytes for the ENTER key (0x13) and end of string (0x00)
			snippets[numOfRecordedSnippets - 1].cmd =(char*) realloc(
			    snippets[numOfRecordedSnippets - 1].cmd,
		...
		if(string[1] >= '0' && string[1] <= (NumOfPorts + '0'))// Valid port number
		{
			...
			if(!strncmp((char*) &string[3],"clicked",7)) {
				...
				if( (button[port].events & BUTTON_EVENT_CLICKED)
				    != BUTTON_EVENT_CLICKED)// Enable the event
				SetButtonEvents(port,1,0,0,0,0,0,0,0,
				BUTTON_EVENT_MODE_OR);
				status =BOS_OK;
			}
				...
				if(!button[port].pressedX1Sec) {
					...
					SetButtonEvents(port,0,0,
					    snippets[numOfRecordedSnippets].cond.buffer1[2],0,
					    0,0,0,0,BUTTON_EVENT_MODE_OR);
					status =BOS_OK;
				}
				...
				if(!button[port].releasedY1Sec) {
					...
					SetButtonEvents(port,0,0,0,0,0,0,0,
					    snippets[numOfRecordedSnippets].cond.buffer1[2],
					    BUTTON_EVENT_MODE_OR);
					status =BOS_OK;
				}
				else {
...
// #3: Module parameter and constant	
					snippets[numOfRecordedSnippets].cond.conditionType =
					SNIP_COND_MODULE_PARAM_CONST;
					snippets[numOfRecordedSnippets].cond.buffer1[1] =modPar1;// Leaving first buffer byte for remote module ID
// Extract the constant
					float constant =atof(thirdPart);
					memcpy(snippets[numOfRecordedSnippets].cond.buffer2,
					    &constant,sizeof(float));// This buffer can be misaligned and cause hardfault on F0
				}

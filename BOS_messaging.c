...
extern BOS_Status SaveEEstreams(uint8_t direction, uint32_t count,
    uint32_t timeout, uint8_t src1, uint8_t dst1, uint8_t src2,
...
BOS_Status SetupDMAStreams(uint8_t direction, uint32_t count, uint32_t timeout,
	...
	if(direction == FORWARD) {
		...
		pdFALSE,(void*) &src,StreamTimerCallback);
		...
		/* Create a timeout timer */
		xTimerStream =xTimerCreate("StreamTimer",pdMS_TO_TICKS(timeout),
		pdFALSE,(void*) &dst,StreamTimerCallback);
		dmaStreamTotal[src - 1] =count;
	}
 ...
 */
BOS_Status BroadcastMessage(uint8_t src, uint8_t dstGroup, uint16_t code,
 ...
 */
BOS_Status SendMessageToGroup(char *group, uint16_t code,
    uint16_t numberOfParams){
	BOS_Status result =BOS_OK;
	uint8_t i =0;
 ...
 */
BOS_Status SendMessageToModule(uint8_t dst, uint16_t code,
    uint16_t numberOfParams){
	BOS_Status result =BOS_OK;
	uint8_t port =0;
		...
		if(AddBcastPayload == true) {
			...
			return BOS_ERR_MSG_DOES_NOT_FIT;
			    ...
			    && ( (numberOfParams + groupMembers + 2)
			        < MAX_PARAMS_PER_MESSAGE))// Multicast
